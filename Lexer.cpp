...
Token Lexer::symbol() {
	...
	if (sym == "print") {
		...
		return Token(eTokenType::FUNCDEF);
	}
	else {
		return Token(eTokenType::SYMBOL, sym);
	}
...
Token Lexer::sign() {
	switch (_source[_pos]) {
		...
		case '{': _pos++; return Token(eTokenType::LBRACE);
		case '}': _pos++; return Token(eTokenType::RBRACE);
		case ',': _pos++; return Token(eTokenType::COMMA);
